...
export async function getEventById(eventId: string): Promise<LimitedEventInfo | null> {
  await connectDB();
  ...
  const isAdmin = await authenticateAdmin();
  const doc = (await Event.findById(eventId).lean()) as RawEvent | null;

  if (!doc) {
    return null;
  }

  // Skip draft events for non-admins
  if (!isAdmin && doc.isDraft) {
    return null;
  }
  ...
  const startDate = new Date(doc.startDate).toISOString();
  const endDate = new Date(doc.endDate).toISOString();
  const registrationDeadline = doc.registrationDeadline ? new Date(doc.registrationDeadline).toISOString() : undefined;
